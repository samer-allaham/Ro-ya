  ...
  return (
    <Box >
      <Flex
        ...
        <Flex flex={{ base: 1 }} justify={{ base: 'right', md: 'end' }}>
...
const DesktopNav = () => {
  ...
  return (
    <Stack direction={'row'} spacing={4}>
      {NAV_ITEMS.slice(0).reverse().map((navItem) => (
...
const DesktopSubNav = ({ label, href, subLabel }: NavItem) => {
  return (
    <Link
      ...
      rounded={'md'}
      className='reversed'
      _hover={{ bg: useColorModeValue('#0476d0', 'gray.900') }}>
      <Stack direction={'row'} align={'center'}>
        <Box>
          ...
          <Text _groupHover={{ color: 'black' }} className='reversed' textAlign={"right"} fontSize={'sm'}>{subLabel}</Text>
        </Box>
        <Flex
          transition={'all .3s ease'}
